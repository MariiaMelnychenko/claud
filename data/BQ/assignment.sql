...
CREATE TABLE IF NOT EXISTS `fast-gate-479912-j1.gold_dataset.financial_metrics` AS
SELECT
...
CREATE TABLE IF NOT EXISTS `fast-gate-479912-j1.gold_dataset.payor_performance` AS
SELECT
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `fast-gate-479912-j1.silver_dataset.claims` c
